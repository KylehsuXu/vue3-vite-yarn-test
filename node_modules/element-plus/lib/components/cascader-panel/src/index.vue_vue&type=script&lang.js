...
var script = vue.defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const lazyLoad = (node$1, cb) => {
      const cfg = config$1.value;
      node$1 = node$1 || new node["default"]({}, cfg, null, true);
      node$1.loading = true;
      const resolve = (dataList) => {
        const parent = node$1.root ? null : node$1;
        dataList && store$1.value.appendNodes(dataList, parent);
        node$1.loading = false;
        node$1.loaded = true;
        node$1.childrenData = node$1.childrenData || [];
        cb && cb(dataList);
      };
    ...
    const getFlattedNodes = (leafOnly) => {
      return store$1.value.getFlattedNodes(leafOnly);
    };
    ...
    const syncCheckedValue = (loaded = false, forced = false) => {
      ...
      if (lazy && !loaded) {
        ...
        const nodes = values.map((val) => store$1.value.getNodeByValue(val)).filter((node) => !!node && !node.loaded && !node.loading);
        if (nodes.length) {
          nodes.forEach((node) => {
        ...
        const nodes = util.deduplicate(values.map((val) => store$1.value.getNodeByValue(val, leafOnly)));
    ...
    const syncMenuState = (newCheckedNodes, reserveExpandingState = true) => {
      ...
      const oldExpandingNode = store$1.value.getSameNode(expandingNode.value);
